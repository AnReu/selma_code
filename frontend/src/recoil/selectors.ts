...
export const indexRequestState = atom<IndexRequest>({
  key: 'indexRequest',
  default: selector<IndexRequest>({
    key: 'indexRequestLoader',
    get: async () => {
...
export interface IndexRequest {
  ...
  plbart: boolean;
  stringBased: boolean;
  neuralApplied: boolean;
  neuralTrained: boolean;
}

export function useIndexRequestMutations() {
  const [, setIndexRequest] = useRecoilState(indexRequestState);

  const sendRequest = async (request: IndexRequest) => {
    const response = await fetch(`${baseURL}/index`,
      {
        method: 'POST',
