def get_eqation_start(string, delimiters):
    ...
    return next(
        (
            (
                string.find(delimiter["begin"]) + len(delimiter["begin"]),
                *tuple(delimiter.values()),
            )
            for delimiter in delimiters
            if string.find(delimiter["begin"]) > -1
        ),
        (-1, "", "", ""),
    )
...
def find_inline_equations(line, inline_delimiter):
    ...
    if eq_start[0] != -1:
        ...
        inline_equations, inline_text = find_inline_equations(
            line[end_eq + len(eq_start[2]) :], inline_delimiter
        )
